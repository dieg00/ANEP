...
def representar_mallado(mallado,title,xlabel='Eje x',ylabel='Eje t',N=1000,M=1000, plot_show=False, x_inicial=0, x_final=100, t_inicial=0, t_final=1):
...
def calcular_lax(delta_x,x0=0,xf=100,t0=0,tf=2.5):
    ...
    for m in range(1,M+1):
        for n in range(1,N):
            u[m,n] = (u[m-1,n+1]+u[n-1,n-1]-parametro_lambda*(f(u[m-1,n+1])-f(u[m-1,n-1])))/2
    representar_mallado(u,title='u(x,t) Lax-Friedrichs', N=N, M=M, plot_show=True, x_inicial=x0, x_final=xf,t_inicial=t0, t_final=tf)
    return u
